...
const AuthForgotPass = () => {
  ...
  const HandleForgotPassword = async e => {
    ...
    if (input.auth) {
      await axios
        .post("http://localhost:5050/api/user/forgot-password", { auth: input.auth })
        .then(res => {
  ...
  const HandleResetPassVaiCode = async e => {
    e.preventDefault();
    await axios
      .post("http://localhost:5050/api/user/pass-recovery-code", { cell: input.auth, verifyCode: input.vcode })
      .then(res => {
            ...
            {msg.status && (
              <>
                ...
                <h6 className={`alert alert-${msg.type}`}>
                  {" "}
                  <p> {msg.message}</p> <span onClick={alertClose}>X</span>
                </h6>{" "}
              </>
            )}
          </div>
            ...
            {formCon && (
              <>
                <form id="phoneverify" onSubmit={HandleResetPassVaiCode} method="POST">
                  <div className="inp-box">
                    <label htmlFor="vcode_fild" className={inpDcon.vcodeL}>
                      Verify Code
                    </label>
